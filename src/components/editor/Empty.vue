<template>
  ...
  <v-row>
    <v-template
      @click="value = { type: 'ConditionalExpression', test: {}, consequent: {}, alternate: {} }"
      >IF/THEN/ELSE</v-template
    >
    <v-template @click="value = { type: 'BinaryExpression', left: {}, right: {}, operator: '<' }"
      >&lt;</v-template
    >
    <v-template @click="value = { type: 'BinaryExpression', left: {}, right: {}, operator: '<=' }"
      >&le;</v-template
    ...
    <v-template @click="value = { type: 'BinaryExpression', left: {}, right: {}, operator: '||' }"
      >OR</v-template
    >
    <v-template @click="value = { type: 'BinaryExpression', argument: {}, operator: '!' }"
      >NOT</v-template
    >
  </v-row>
