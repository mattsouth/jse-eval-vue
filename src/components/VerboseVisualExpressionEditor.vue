...
<template>
  <template v-if="!modelValue.type" :class="{'opacity-50': disabled}">
    ...
    <v-row hide-delete>
      <v-template @click="enterValue({type: 'ConditionalExpression', test: {}, consequent: {}, alternate: {}})">IF/THEN/ELSE</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'<', left: {}, right: {}})">&lt;</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'<=', left: {}, right: {}})">&le;</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'===', left: {}, right: {}})">=</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'>=', left: {}, right: {}})">&ge;</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'>', left: {}, right: {}})">&gt;</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'!==', left: {}, right: {}})">!=</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'&&', left: {}, right: {}})">AND</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'||', left: {}, right: {}})">OR</v-template>
      <v-template @click="enterValue({type: 'UnaryExpression', operator:'!', argument: {}})">NOT</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'+', left: {}, right: {}})">+</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'-', left: {}, right: {}})">-</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'*', left: {}, right: {}})">×</v-template>
      <v-template @click="enterValue({type: 'BinaryExpression', operator:'/', left: {}, right: {}})">÷</v-template>
  ...
  <v-block v-if="modelValue.type == 'ConditionalExpression'" :depth="depth" :disabled="disabled">
    <v-row @delete="deleteRow" @delete-sub="deleteSub"><span class="badge bg-secondary me-2">IF</span></v-row>
    <v-expr-viewer v-model="modelValue['test']" :depth="this.depth + 1" att="test" @delete-sub="deleteSub" @update-sub="updateSub"/>
    <v-row hide-delete><span class="badge bg-secondary me-2">THEN</span></v-row>
    <v-expr-viewer v-model="modelValue['consequent']" :depth="this.depth + 1" att="consequent" @delete-sub="deleteSub" @update-sub="updateSub" />
    <v-row hide-delete><span class="badge bg-secondary me-2">ELSE</span></v-row>
    <v-expr-viewer v-model="modelValue['alternate']" :depth="this.depth + 1" att="alternate" @delete-sub="deleteSub" @update-sub="updateSub"/>
  </v-block>
  <!-- binary expression -->
  <template v-if="modelValue.type == 'BinaryExpression'">
    ...
    <template v-if="isLogical(modelValue.operator) && modelValue.operator==operator">
      <div class="d-block" :class="{'opacity-50': disabled}">
        <v-expr-viewer v-model="modelValue['left']" :depth="this.depth" :operator="modelValue.operator" att="left" @delete="deleteRow" @delete-sub="deleteSub" @update-sub="updateSub"/>
      </div>
      <div class="d-block" :class="{'opacity-50': disabled}">
        <v-expr-viewer v-model="modelValue['right']" :depth="this.depth" :operator="modelValue.operator" att="right" @delete="deleteRow" @delete-sub="deleteSub" @update-sub="updateSub"/>
      </div>
    </template>
    <template v-else>
      ...
      <v-block :depth="depth" :disabled="disabled">
        ...
        <div class="d-block">
          <v-expr-viewer v-model="modelValue['left']" att="left" :depth="this.depth + 1" :operator="modelValue.operator" @delete="deleteRow" @delete-sub="deleteSub" @update-sub="updateSub"/>
        </div>
        <div class="d-block">
          <v-expr-viewer v-model="modelValue['right']" att="right" :depth="this.depth + 1" :operator="modelValue.operator"  @delete="deleteRow" @delete-sub="deleteSub" @update-sub="updateSub"/>
        </div>
      </v-block>
    </template>
  </template>
  ...
  <template v-if="modelValue.type == 'UnaryExpression'">
    <v-block :depth="depth" :disabled="disabled">
      ...
      <div class="d-block">
        <v-expr-viewer v-model="modelValue['argument']" att="argument" :depth="this.depth + 1" @delete="deleteRow" @delete-sub="deleteSub" @update-sub="updateSub"/>
      </div>
    </v-block>
  </template>
  <v-row v-if="modelValue.type == 'Identifier'" @delete="deleteRow" @delete-sub="deleteSub" @update-sub="updateSub"> 
    ...
    <span v-else><em>{{ modelValue.name }}</em></span>
  </v-row>
